...
export default function ProjectCard({ project, index }: { project: Project; index: number }) {
  ...
  useGSAP(() => {
    ...
    const onMove = (event: PointerEvent) => {
      ...
      xTo(x * 6)
      yTo(y * -6)
      glow.current?.style.setProperty('background', `radial-gradient(circle at ${(x + 0.5) * 100}% ${(y + 0.5) * 100}%, rgba(201, 168, 108, 0.12), transparent 50%)`)
    }
  ...
  return (
    ...
    >
      ...
      <div className="relative flex items-stretch p-4 lg:p-5 gap-5">
        <div className="relative flex-shrink-0 w-20 h-20 lg:w-24 lg:h-24 rounded-lg overflow-hidden transition-transform duration-500" style={{ transform: hovered ? 'scale(1.02)' : 'scale(1)' }}>
          <div className="absolute inset-0" style={{ background: 'linear-gradient(135deg, rgba(255,255,255,.05), rgba(22,22,22,.8))', border: '1px solid rgba(255,255,255,.08)' }} />
          <img src={project.thumbnailSrc} alt={`${project.title} thumbnail`} className="absolute inset-0 h-full w-full object-cover" />
        </div>
        <div className="flex-1 flex flex-col justify-center min-w-0 pr-8">
          <h3 className="font-sans text-lg lg:text-xl text-cream font-medium tracking-tight mb-1.5 truncate">{project.title}</h3>
          <p className="font-sans text-sm leading-relaxed mb-2 line-clamp-2 text-stone">{project.summary}</p>
          <span className="font-sans text-xs tracking-wide text-gold/70">{project.tech.slice(0, 4).join(' / ')}</span>
        </div>
