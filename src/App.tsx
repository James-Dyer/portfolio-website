...
function RouteSignal() {
  ...
  useGSAP(() => {
    ...
    gsap.timeline()
      .set(root.current, { display: 'grid' })
      .fromTo(root.current, { clipPath: 'inset(0 0 100% 0)' }, { clipPath: 'inset(0 0 0% 0)', duration: 0.28, ease: 'power3.inOut' })
      .from('.route-signal-line', { scaleX: 0, transformOrigin: 'left', duration: 0.22 }, '<0.08')
      .to(root.current, { clipPath: 'inset(0 0 0 100%)', duration: 0.42, ease: 'power3.inOut', delay: 0.12 })
      .set(root.current, { display: 'none' })
  }, { scope: root })

  return <div ref={root} className="route-signal" aria-hidden="true"><span>Routing signal / {location.pathname || '/'}</span><i className="route-signal-line" /></div>
}
...
function HomePage() {
  ...
  useGSAP(() => {
    ...
    gsap.to(intro, {
      scrambleText: {
        text: 'Building production-grade backend, full-stack, and applied AI systems.',
        chars: '01/.',
      },
      delay: 0.45,
      duration: 1.2,
      onComplete: () => {
        intro.innerHTML = originalMarkup
        gsap.fromTo(
          intro.querySelectorAll('.hero-highlight'),
          { color: 'var(--color-stone)' },
          { color: 'var(--color-cream)', duration: 0.65, stagger: 0.08, ease: 'power2.out' },
        )
      },
    })
  }, { scope: root })
  ...
  return (
    <div ref={root} className="relative bg-ink">
      ...
      <MagneticShapes />
      <div className="fixed inset-0 pointer-events-none" style={{ zIndex: 5, background: 'radial-gradient(circle at 65% 40%, rgba(0, 0, 0, 0.85) 0%, rgba(0, 0, 0, 0.8) 18%, rgba(0, 0, 0, 0.65) 30%, transparent 40%)' }} />
      <div className="fixed inset-0 pointer-events-none">
        <div className="absolute w-[800px] h-[800px] rounded-full" style={{ background: 'radial-gradient(circle, var(--color-accent-dim) 0%, transparent 70%)', top: '-20%', right: '-10%', filter: 'blur(100px)', opacity: 0.6 }} />
      </div>
      ...
      <div className="relative z-10 flex flex-col lg:flex-row lg:pl-16 xl:pl-28">
        <div className="lg:w-[40%] xl:w-[35%] lg:sticky lg:top-0 lg:h-screen flex items-start justify-center px-4 lg:px-8 xl:px-10 py-8 lg:py-12 pb-12">
          <div className="card-glow bg-cream rounded-2xl p-8 pb-20 lg:p-12 lg:pb-24 w-full max-w-md relative overflow-hidden animate-scale-in delay-200">
            ...
            <div className="absolute bottom-4 right-4 w-8 h-8 pointer-events-none"><div className="absolute bottom-0 right-0 w-full h-0.5 bg-gold/40" /><div className="absolute bottom-0 right-0 w-0.5 h-full bg-gold/40" /></div>
            <div className="absolute -right-12 top-1/3 w-40 h-px bg-gradient-to-r from-transparent via-gold/20 to-transparent rotate-45 pointer-events-none" />
            <div className="relative mb-8">
              <div className="aspect-[4/5] bg-ink rounded-xl overflow-hidden">
                <picture>
                  <source srcSet={`${BASE_URL}DSC03663_480.avif`} type="image/avif" />
                  <img src={`${BASE_URL}DSC03663.JPG`} alt="Portrait of James Dyer" className="w-full h-full object-cover" loading="lazy" decoding="async" />
                </picture>
              </div>
            ...
            <h2 className="font-display text-4xl lg:text-5xl text-ink font-medium tracking-tight mb-2">James Dyer</h2>
            <p className="font-body text-sm lg:text-base text-ink/70 tracking-wide">New Graduate Software Engineer</p>
            <div className="absolute inset-x-0 bottom-6 lg:bottom-8 flex justify-center gap-8">
              <a href="https://github.com/James-Dyer" target="_blank" rel="noopener noreferrer" className="text-ink hover:bg-ink/10 rounded-lg p-2 -m-2 transition-all duration-300" aria-label="GitHub"><svg viewBox="0 0 24 24" className="h-7 w-7 fill-ink"><path fillRule="evenodd" d="M12 2a10 10 0 0 0-3.162 19.492c.5.092.682-.218.682-.484 0-.237-.009-.868-.014-1.703-2.775.603-3.36-1.337-3.36-1.337-.454-1.152-1.11-1.46-1.11-1.46-.907-.62.069-.607.069-.607 1.003.07 1.53 1.03 1.53 1.03.892 1.528 2.341 1.087 2.91.832.092-.647.35-1.087.637-1.338-2.216-.252-4.547-1.108-4.547-4.934 0-1.09.39-1.983 1.029-2.681-.103-.253-.446-1.27.098-2.646 0 0 .84-.269 2.75 1.025A9.564 9.564 0 0 1 12 6.844a9.56 9.56 0 0 1 2.504.337c1.909-1.294 2.748-1.025 2.748-1.025.546 1.376.202 2.393.1 2.646.64.698 1.028 1.59 1.028 2.68 0 3.836-2.335 4.678-4.558 4.925.359.31.679.921.679 1.856 0 1.339-.012 2.419-.012 2.749 0 .268.18.58.688.482A10 10 0 0 0 12 2Z" clipRule="evenodd" /></svg></a>
              <a href="https://linkedin.com/in/jamesthedyer" target="_blank" rel="noopener noreferrer" className="text-ink hover:bg-ink/10 rounded-lg p-2 -m-2 transition-all duration-300" aria-label="LinkedIn"><svg viewBox="0 0 24 24" className="h-7 w-7 fill-ink"><path d="M20.452 20.452h-3.555v-5.569c0-1.328-.027-3.036-1.85-3.036-1.851 0-2.135 1.445-2.135 2.939v5.666H9.357V9h3.413v1.561h.049c.476-.9 1.637-1.85 3.369-1.85 3.604 0 4.27 2.372 4.27 5.455v6.286ZM5.337 7.433a2.064 2.064 0 1 1 .001-4.128 2.064 2.064 0 0 1-.001 4.128Zm1.779 13.019H3.556V9h3.56v11.452Z" /></svg></a>
              <a href="mailto:james.dyer.dyer@gmail.com" className="text-ink hover:bg-ink/10 rounded-lg p-2 -m-2 transition-all duration-300" aria-label="Email"><svg viewBox="0 0 24 24" className="h-7 w-7 fill-ink"><path d="M3 6.75A2.75 2.75 0 0 1 5.75 4h12.5A2.75 2.75 0 0 1 21 6.75v10.5A2.75 2.75 0 0 1 18.25 20H5.75A2.75 2.75 0 0 1 3 17.25Zm2.75-1.25a1.25 1.25 0 0 0-1.25 1.25v.394l7.084 3.798a.75.75 0 0 0 .732 0L19.4 7.144v-.394a1.25 1.25 0 0 0-1.25-1.25Zm13 3.178-5.932 3.183a2.25 2.25 0 0 1-2.136 0L4.75 8.678v8.572a1.25 1.25 0 0 0 1.25 1.25h12.5a1.25 1.25 0 0 0 1.25-1.25Z" /></svg></a>
            </div>
          </div>
        </div>
          ...
          <section className="min-h-screen flex flex-col justify-start px-4 lg:px-8 xl:px-10 pt-12 lg:pt-16 pb-10">
            <div className="mt-0">
              ...
              <p className="hero-intro font-body text-stone text-lg lg:text-xl max-w-xl leading-relaxed animate-slide-up delay-500">
                Building production-grade <span className="hero-highlight text-cream">backend</span>, <span className="hero-highlight text-cream">full-stack</span>, and <span className="hero-highlight text-cream">applied AI systems</span>.
              </p>
